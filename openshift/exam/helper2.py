...
from openshift.exam.models import Question

import datetime
def create_exam(usr,examtype):
	curr_exam=ExamAttended(exam=examtype,user=usr)#create new exam
	curr_exam.save()
	sec_left=10800
	return (curr_exam,sec_left)
		
def start_exam(usr,examtype):
	try:curr_exam = ExamAttended.objects.latest("start_time")
	except:return create_exam(usr,examtype)
	dur_exam=datetime.timedelta(0,10800,0)
	#check did the last exam time full expired
	if (curr_exam.start_time+dur_exam) < datetime.datetime.today():	return create_exam(usr,examtype)		
	#if last exam not full expired return last_exam and time left
	timeleft=datetime.datetime.today()-curr_exam.start_time
	sec_left=10800-timeleft.seconds
	return (curr_exam,sec_left)
